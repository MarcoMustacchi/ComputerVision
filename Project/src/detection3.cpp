...
void detection(cv::Mat& img)
{
    ...
    int windowSizeWidth = std::get<0>(INITIAL_WINDOW_SIZE);
    int windowSizeHeigth = std::get<1>(INITIAL_WINDOW_SIZE);
    
    //Compute the Stride for the Rows and Cols
	int strideRows = windowSizeWidth * STRIDE_ROWS_FACTOR;
	int strideCols = windowSizeHeigth * STRIDE_COLS_FACTOR;
	
	cout << strideRows << endl;
	cout << strideCols << endl;
	
	int i = 0;

	for (int row = 0; row < img.rows - windowSizeHeigth; row += 10)
	{
		//Range of rows coordinates
		cv::Range rowRange(row, row + windowSizeHeigth);
		
		for (int col = 0; col < img.cols - windowSizeWidth; col += 10)
		{			
